...
### Added
- `makecrew doctor` 就绪检查：分别报告本地工作区、全局入口、Skill、方法目录、宿主回调、RAG 和运行时探测状态；低副作用初始化最小元数据，不创建员工/对话、不安装 Skill、不修改全局入口。
- 方法发现增强：新增理论-案例-反例研究、可迁移对标筛选、内容资产单元/主题地图、发布前双层检查和导航-原件-增量审计方法卡；每张卡提供稳定 ID、适用边界、交付物、验收证据和成本提示。
- 方法发现质量增强：按任务信号排序本地与外部方法，给宿主候选补稳定 ID、匹配理由和搜索总量统计；单条异常候选不会中断发现，并新增 `makecrew method-audit` 目录检查。
- 借鉴记录：登记 `dbskill` v2.18.39（CC BY-NC 4.0）为公开机制参考，不整包安装、不复制源码或知识原文。
