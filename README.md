...
```text
模糊想法 -> 最多三问 -> 方法/Skill 发现 -> 候选方案与取舍 -> 用户确认 -> 当前对话执行 -> 验收 -> 自进化记录
```
...
每条任务还会生成一份可序列化的工作流图：需求澄清、方法发现、用户确认、专业执行、独立验收、交付和学习都有明确节点。独立员工节点共享同一确认门并行运行，验收节点等待全部分支；重启时可从最近检查点继续。公开发布、付款、删除等动作会停在可见的人审中断点，不把确认藏在员工提示词里。详见 [`docs/inspiration-comparison.md`](docs/inspiration-comparison.md)。

验收后的自进化同样是证据驱动的：记录评分、反馈和根因，重复问题才生成提案，再用历史任务回放比较基线与候选。候选没有提升就保留现状；即使通过回放，也由用户/主管审阅后按版本采用，不会静默修改员工能力。
