...
MakeCrew 是一个跨平台的 AI 工作入口和协作内核：把自然语言需求变成可路由、可协作、可验收、可恢复的工作。它先理解需求，再检查本地已安装的 Skill 和方法；本地有匹配就直接使用，缺少关键能力就搜索候选并交给用户选择。单任务留在当前对话内，只有多任务、依赖或跨项目决策才启用主管/CEO 调度。

它不是固定数量的“虚拟员工聊天群”，也不是绑定某个模型的运行时。用户可以从三个核心岗位开始，按项目增加开发、研究、内容、设计、知识库或 Skill 员工；Codex、Claude、Gemini 和自建 Agent 平台都可以通过适配器接入真实工具和对话。
...
| 员工数量由用户决定 | 三个核心岗位即可运行，后续按项目增加专家 | 核心岗位保护、自定义员工、缺岗时临时员工 |
| 旧工作区可增量升级 | 保留已有员工、记忆和配置，只补齐缺失项 | 非破坏式 bootstrap 与核心岗位保护 |
