...
> **可安装的开源 AI 工作框架：让 AI 先理解需求，再匹配 Skill、调用工具、按需协作，并交付可验证结果。**

MakeCrew 以 Skill 形式接入 Codex、Claude、Gemini 或自建 Agent 平台，补上“有能力但不会组织工作”的一层：它把模糊想法变成清晰任务，把本地已有能力和方法匹配到当前需求，再根据复杂度选择单对话、临时专家组或多任务并行，最后留下验收证据、可恢复状态和可审查的改进记录。
...
MakeCrew 适合想把 AI 从“聊天回答”升级为“可靠完成工作”的人：

- 刚开始使用 AI、经常不知道该怎样描述需求的新手；
- 独立开发者、产品经理、研究者和内容创作者；
- 同时维护多个网站、应用、内容或研究项目的个人与小团队；
- 已经安装很多 Skill，却遇到不会调用、上下文混乱、重复解释和结果难验收的人；
- 希望保留现有对话和员工，再逐步增加项目化协作能力的 Codex/Claude 用户。

## 三步开始

1. 把本仓库交给你的 AI：<https://github.com/GodMaking/makecrew>。
2. 让它读取 `README.md`、`skills/makecrew/SKILL.md` 和 `docs/getting-started.md`，盘点宿主已有 Skill、工具、对话和项目记忆。
3. 让它按安装提示词完成增量配置，并用清晰单任务、缺少 Skill、模糊需求、多任务批次四项小测试报告真实结果。

安装后，用户可以主动输入 `$makecrew` 或 `$task-intake`；也可以配置全局入口，让网站、应用、产品、视频、文档和自动化等多步骤新任务自动先走需求澄清与能力匹配。简单查询和状态检查仍保持直达。
...
一个任务留在当前对话；多个独立任务或存在依赖时才规划主管/CEO 批次。
