...
- 需求存在实质缺口时，每轮只问 1-3 个会改变结果的问题；问题总数为 0-N，直到目标用户、交付物、现有基础、关键约束和验收标准达到可执行清晰度。用户回答后必须逐项回填问题 ID 和答案，再重新检查剩余缺口；不能把“三个问题”当成总上限。
- 需求清楚后，先列出理解结果、选用的 Skill/工具、工作流程、交付物、验收标准、成本控制和主要风险。只有这些内容齐全且 Skill/工具匹配完成后，才可以询问是否确认执行；不能只发一句“是否确认执行”。
