...
`scope_for_employee(employee_id, project_ids=...)` 将注册表身份转换成检索作用域。派发任务时，作用域会随最小任务包传给宿主：

- `CEO-001`：公司记忆，可附带指定项目；
- `PM-001`：项目记忆与公司规则；
- 专业员工：绑定项目的任务上下文；
- `KNO-001`、`SKL-001` 等公司与项目岗位：只有显式声明项目后才读取项目记录。

任务包只包含作用域参数，不包含知识正文。宿主检索器应在查询前执行同样的权限过滤。
...
`rag-audit` 还会报告重复片段、同标题冲突来源、源文件哈希漂移、丢失来源和孤儿记录。报告只供人工处理，不会自动删除或标记任何记录。
