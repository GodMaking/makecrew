...
自进化层是事件触发的改进闭环，不是每个任务都要支付的固定步骤。用户差评、验收失败、返工、重复问题或明确要求复盘时，才把评分、反馈和根因转成可审阅的改进提案，再用历史任务回放验证提案效果。普通成功任务结束于交付。

## 循环

```text
触发信号 -> 记录反馈/根因 -> 聚合同类证据 -> 生成提案
                                           -> 历史回放
                                           -> 候选平均分更高才采用
```
...
`task-intake` 只在任务需要时展示发现到的方法和 Skill；出现学习触发信号后，
再将评分、反馈和根因写入 `LearningEngine`。只有重复问题才生成提案，提案经过代表性
