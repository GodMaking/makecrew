...
| 识别结果 | 触发信号 | 实际路径 | 用户确认 |
|---|---|---|---|
| `direct` | 目标清楚、常规、可回退 | 本地 Skill/方法匹配 -> 执行 -> 验收 -> 交付 | 已安装 Skill 直接使用 |
| `clarify` | 缺少会改变结果的信息 | 每轮询问 1-3 个关键问题 -> 检查剩余缺口 -> 按需继续 | 回答问题或授权 AI 决定 |
| `discovery` | 本地无匹配、能力有缺口，或要求最新搜索/比较 | 外部方法发现 -> 比较 -> 执行 -> 验收 -> 交付 | 同时要求先看方案时确认 |
| `ready_for_skill_choice` | 本地缺少任务所需 Skill，且搜到外部候选 | 展示候选用途/来源 -> 用户选择安装/使用 -> 执行 | 需要选择候选 |
| `plan_first` | 用户明确说“先给方案”“先别执行”等 | 形成方案 -> 用户确认 -> 执行 -> 验收 -> 交付 | 需要一次确认 |
| `product_delivery` | 新建或大幅重做网站、应用、产品 | 需求澄清 -> 产品简报 -> Demo/原型 -> 技术方案 -> 用户确认 -> 实现 -> 验收 | 实现前需要确认 |
...
- 常规任务使用本地清单完成轻量匹配，跳过外部搜索、候选比较和确认往返。
- 已安装且匹配的 Skill 直接使用；实质能力缺口才扩大搜索。
...
每个问题都有稳定 `question_id`。宿主应同时把答案通过 `answers={question_id: value}` 传回；只传 ID 的旧调用仍可兼容，但会标记答案内容未回填。下一轮会跳过已解决问题；`clarification.max_total_questions` 为 `null`，表示总数没有硬上限。内置规则提供通用缺口，宿主 AI 还可以通过 `material_gaps` 增加任意数量的领域专属问题，系统负责去重和分轮。
